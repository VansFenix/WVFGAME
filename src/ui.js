...
import { init2048 } from './games/game2048.js';
...
export function showHelp() {
  ...
  const rules = {
    ...
    '2048': ['🔢 2048', ['<kbd>←↑↓→</kbd> или свайп — двигайте плитки', 'Соединяйте плитки с одинаковыми числами', 'Цель — собрать плитку 2048', 'После каждого хода появляется новая плитка', 'Игра заканчивается, когда поле заполнится']],
    'flappy': ['🐦 Flappy Bird', ['<kbd>Space</kbd> / клик / тап — махнуть крыльями', 'Пролетайте между трубами, не задевая их', 'Каждая пройденная труба = +1 очко', 'Не падайте на землю и не врезайтесь в трубы']]
  };
...
export function startGame(type) {
  ...
  switch (type) {
    ...
    case '2048': DOM.gameTitle.textContent = '🔢 2048'; init2048(); break;
    case 'flappy': DOM.gameTitle.textContent = '🐦 Flappy Bird'; initFlappy(); break;
  }
}
